  ...
  describe "#validation" do
    ...
    context "正常系" do
      it "必要な情報が入力されている場合、保存に成功すること" do
        post = Post.new(title: "テスト", content: "テスト", mood: "happy", user: user)
        expect(post).to be_valid # 有効であることを確認
      end

      it "タイトルが50文字の場合、保存に成功すること" do
        post = Post.new(title: "あ" * 50, content: "テスト", mood: "happy", user: user)
        expect(post).to be_valid 
      end

      it "内容が500文字の場合、保存に成功すること" do
        post = Post.new(title: "テスト", content: "あ" * 500, mood: "happy", user: user)
        expect(post).to be_valid
      end
    end
    ...
    context "異常系" do
      it "タイトルが空欄の場合、エラーが表示されること" do
        post = Post.new(title: "", content: "テスト", mood: "happy", user: user)
        post.valid?
